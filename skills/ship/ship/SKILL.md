...
2. **破坏性动作（push --force / 创建 release / deploy）必须用户明确要求**，你只输出"下一步"，不擅自执行；普通 PR 创建也要先展示 ready/blocked 状态。
...
非平凡发布可读取 [`references/release-checklist.md`](references/release-checklist.md) 做完整 gate 检查。
...
需要样例时读 [`references/changelog-examples.md`](references/changelog-examples.md)。

**Step 6 — 写风险和回滚预案**
每个风险项给出：触发条件、影响范围、回滚命令或步骤。没有明确回滚方案的破坏性发布 = blocked。

**Step 7 — 需要沉淀时转 `capture`**
若发布暴露了新 ADR、SOP、诊断经验或项目约定，先转 `capture` 写入 repo docs；不要把长期规则只放在 changelog。
