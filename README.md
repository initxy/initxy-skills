...
- **每个 skill 小而独立**：一个 skill 只做一件事，可组合，不绑定特定工具；尽量单文件，细节多了才拆 `references/`。
...
- **上下文干净**：主文件加载即用，无需跳读；细节拆进 `references/` 时也只在用到那一步才读，主文件始终精简。
...
- `write-zh`：引导式中文写作。逐节带你把文档写出来，去 AI 味、多配图，贴合第一性原理为主、传统文化淡点缀的个人风格。主文件管流程，细节拆成三份按需读取的 references：`techniques`（写作技巧）、`de-ai`（去 AI 味清单）、`styles`（五种可切换的语言风格画像：清淡科普、老练实战、第一性硬核、口语个人、严谨工程）。

## 结构

每个 skill 位于 `skills/<name>/SKILL.md`，符合 skills.sh / Agent Skills 格式。多数 skill 单文件即可；个别（如 `write-zh`）把不常用的细节拆进同目录的 `references/`，由主文件按需引用，主文件本身保持精简。
