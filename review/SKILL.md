...
2. 自审不可信——你已知道为什么这么写，必须 spawn skeptic subagent 或调 `grill` 消除盲点。
...
| 模式 | 触发 | 关键差异 |
|---|---|---|
| **自审** | 你刚写的代码 | 必须 spawn skeptic subagent；你知道意图，subagent 只看 diff，专门找漏洞 |
| **PR 审** | 别人的代码 | 不需要 subagent；先读 plan/spec/issue 理解意图，再直接审 |

自审时调用 `grill` skill，把 diff 作为产物传入。skeptic subagent 的 prompt 模板**单一来源**在 `grill/SKILL.md`，本 skill 不内嵌副本，避免两份漂移。

## 流程

1. **查看变更**：`git diff HEAD` 或 `gh pr diff <PR>`，列出所有变更文件。
2. **理解意图**：读 plan/spec/issue，明确这段代码要解决什么问题、预期行为是什么。
3. **自审模式**：spawn skeptic subagent（见上方模板），等待攻击结果后纳入报告。
4. **逐项检查**：
   - 正确性：逻辑、边界条件、类型断言。
